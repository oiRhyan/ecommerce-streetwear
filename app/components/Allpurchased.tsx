    ...
    if(purchaseProducts.length === 0){
        return(
            <div className='relative flex items-center justify-center'>
                ...
                <h1 className='absolute top-[80%] text-2xl text-orange-600'>Pedidos</h1>
            </div>
        )
    }
    return (
        ...
        {purchaseProducts.map((cartProduct) => (
            <div key={cartProduct?.id} className='flex items-center justify-between w-8/12 mx-auto shadow-lg p-5 rounded-lg mt-6'>
                <div>
                    ...
                    <h2 className='mb-2 text-neutral-800'>Preço: {cartProduct?.price}</h2>
                    <h3 className='text-sm text-neutral-600 mb-2'>Estilo: {cartProduct?.category}</h3>
                    <h3 className='text-sm text-neutral-600 mb-2'>Modelo: {cartProduct?.style}</h3>
                    <h3 className='text-sm text-neutral-600 mb-2'>Drop: {cartProduct?.store}</h3>
                    <p className='text-green-600'> Comprado! <BsCheck2All size={20} className='text-green-600'/></p>
                </div>
