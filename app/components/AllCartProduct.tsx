...
const AllCartProduct = async (props: Props) => {
    const allcartproduct = await prisma.carrinho.findMany({
        where:{
            userId:props.userId
        }
    })
    ...
    if(cartProducts.length === 0){
        return(
            <div className='relative flex items-center justify-center'>
                ...
                <h1 className='absolute top-[80%] text-2xl text-orange-600'> Carrinho Vazio </h1>
            </div>
        )
    }
    return (
        ...
        {cartProducts.map((cartProduct) => (
            <div key={cartProduct?.id} className='flex items-center justify-between w-8/12 mx-auto shadow-lg p-5 rounded-lg mt-6'>
                <div>
                    ...
                    <h2 className='mb-2 text-white'>Preço: {cartProduct?.price}</h2>
